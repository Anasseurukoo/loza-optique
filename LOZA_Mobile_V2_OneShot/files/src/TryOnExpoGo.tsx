...
export default function TryOnExpoGo({
    ...
    return (
      <View style={styles.permission}>
        ...
        <Text style={styles.permissionCopy}>
          Autorisez la caméra frontale. Dans Expo Go, vous pouvez calibrer manuellement la
          monture. Le suivi automatique est activé dans l’APK de développement.
        </Text>
  ...
  const width = Math.min(SCREEN_WIDTH * 0.9, SCREEN_WIDTH * 0.66 * physicalRatio * scale);
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={styles.controls}>
        <Pressable style={styles.controlButton} onPress={() => setScale((value) => Math.max(0.72, value - 0.04))}>
        ...
        <Pressable style={styles.controlButton} onPress={() => setOffset({ x: 0, y: 0 })}>
          <Ionicons name="locate-outline" size={18} color="#D8BF82" />
        </Pressable>
        <Pressable style={styles.controlButton} onPress={() => setOffset((value) => ({ ...value, y: value.y + 4 }))}>
          <Ionicons name="arrow-down" size={18} color="#fff" />
        </Pressable>
        <Pressable style={styles.controlButton} onPress={() => setScale((value) => Math.min(1.35, value + 0.04))}>
      ...
      <View style={styles.bottom}>
        <Text style={styles.activeLabel}>{active.code} · {active.measurements.lens}-{active.measurements.bridge}</Text>
        ...
        <Text style={styles.privacy}>Aucune image envoyée · traitement local</Text>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  tint: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(7,29,34,.08)' },
  faceGuide: {
    position: 'absolute',
    left: '16%',
    top: '17%',
